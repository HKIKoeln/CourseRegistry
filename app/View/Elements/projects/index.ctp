...
<div class="scroll_wrapper">
	<table>
		...
		if(!empty($records)) {
			$showDetails = (count($records) === 1) ? true : false;
			foreach($records as $k => $record) {
				$classname = ($k % 2 == 0) ? 'even' : 'odd';
				$this->set(compact('classname', 'record', 'showDetails', 'k'));
				echo $this->element('projects/table/row');
				if(!empty($detailsFieldlist)) echo $this->element('table/row_details');
			}
		}
		?>
	</table>
